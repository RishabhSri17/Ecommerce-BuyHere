...
const OrderDetailsPage = () => {
	...
	const paymentHandler = async (e) => {
		...
		try {
			...
			// Safety check for Razorpay API key
			if (!razorpayApiKey?.config?.razorpayKeyId) {
				toast.error("Payment gateway configuration is not available");
				return;
			}

			const razorpayData = {
				amount: Math.round(order.totalPrice * 100), // Ensure amount is in paise and rounded
				currency: "INR",
				...
				handler: async (response) => {
					try {
						...
						const details = {
							...
							updateTime: validationData.result.updateTime,
